---
title: "Lectures Julien"
output: 
  flexdashboard::flex_dashboard:
    theme: lumen
    orientation: rows
    vertical_layout: fill
---

```{r setup, include=FALSE}
library(tidyverse)
library(DT)
library(crosstalk)
library(fontawesome)
library(viridis)
library(plotly)
library(packcircles)
library(flexdashboard)
library(ggplot2)
library(highcharter)
library(forcats)
library(sf)
library(leaflet)
```

```{r}
knitr::opts_chunk$set(echo = F)
knitr::opts_chunk$set(warning = F)
knitr::opts_chunk$set(message = F)
```




```{r}
# Fichier source de données

data <- read.csv("https://framagit.org/Mac_Graveur/lectures/-/raw/main/liste-extra-wikidata_iso.csv", encoding = "UTF-8")

st <- SharedData$new(data)

```


Total des lectures
=======================================

Column {.sidebar data-width=650}
------------------------------------------------

### Lectures mensuelles 2020-2025

```{r}
mois <- data %>% 
  na.omit(mois_début) %>%
  group_by(Année_début) %>%
  count(mois_début) %>%mutate(mois_début = recode(mois_début,
            "1" = "Ja",
            "2" = "F",
            "3" = "Ma",
            "4" = "Av",
            "5" = "Mai",
            "6" = "Juin",
            "7" = "Juil",
            "8" = "Ao",
            "9" = "S",
            "10" = "O",
            "11" = "N",
            "12" = "D"))

mois$mois_début <- factor(mois$mois_début, levels = c("Ja", "F", "Ma", "Av", "Mai", "Juin", "Juil", "Ao", "S", "O", "N", "D"))

mois$Année_début <- factor(mois$Année_début)

 plotmois <- mois %>%
  ggplot( aes(x=mois_début, y=n, group=Année_début, color=Année_début, text = paste("Mois de début : ", mois_début, "\nNombre : ", n))) +
    geom_line() +
    scale_color_viridis(discrete = TRUE, name = "Année") +
    theme(
      legend.position="none",
      plot.title = element_text(size=14)
    ) +
    ggtitle("Nombre de lecture par mois") +
    theme_minimal()+
    xlab("Mois de début de lecture") +
    ylab ("Nombre")
 
ggplotly(plotmois, tooltip = "text")
```




Row {data-width=450}
---------------------------------------

### Nombre de livres lus 

```{r}

nbr <- data %>% 
        reframe(n())

valueBox(
  value = nbr, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages <- data %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages, 
  icon = "fa-file")

```

Row {.tabset data-width=450}
-------------------------------------------------

### Genre

```{r}

genre <- data %>% 
    mutate(Genre.auteur = recode(Genre.auteur,
           "C" = "Collectif",
           "F" = "Femme",
           "H" = "Homme",
           "M" = "Homme",
           "NB" = "Non binaire")) %>% 
    count(Genre.auteur, Année_début)

genre <- ggplot(genre, aes(fill= Genre.auteur, y=n, x=factor(Année_début, levels= c("2020", "2021", "2022", "2023", "2024", "2025", "Avant 2020")), text = paste("Nombre : ", n, "\nType :", Genre.auteur))) +
  geom_bar(position = "stack", stat = "identity" ) +
  ggtitle("Répartition par genres et par années") +
   labs(
        x = "Année de lecture",
        y = "Nombre de personnes",
        fill = "Légende"
    ) +
  scale_fill_viridis(discrete = T, option = "viridis") +
  theme_minimal()

ggplotly(genre, tooltip = "text")

```


### Top des autrices et auteurs

```{r}
auteurs <- data %>% 
  group_by(Auteur) %>% 
  count(Auteur) %>% 
  arrange(desc(n)) %>% 
  head(28)

packing <- circleProgressiveLayout(auteurs$n, sizetype="area")

auteurs <- cbind.data.frame(auteurs, packing)

myPlotCord <- circleLayoutVertices(packing)

pl <- ggplot() + 
  geom_polygon(data = myPlotCord, aes(x, y, group = id, fill= as.factor(id))) + 
  geom_text(data = auteurs, aes(x, y, size=n, label = paste0(Auteur), text = paste("Auteur : ", Auteur, "<br>", "Nombre : ", n))) + 
  scale_size_continuous(range = c(2,4)) + 
  coord_equal() + 
  theme_void() +
  theme(legend.position="none",
        panel.border = element_blank(),
        panel.grid.major = element_blank()) + 
  labs(title ="Top des autrices et des auteurs") +
  labs(subtitle ="Population") +
  scale_fill_viridis(discrete = T, option = "viridis")


ggplotly(pl, tooltip = "text")



```


### Nationalités

```{r}
natio <- data %>% 
  group_by (Nationalité) %>%
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>% 
  hc_title(text = "Nationalités des auteurs et autrices") %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Top des maisons d'édition

```{r}
edit <- data %>% 
  group_by(Editeur) %>% 
  count(Editeur) %>% 
  arrange(desc(n)) %>% 
  head(10) %>%
  ggplot(aes(x=reorder(Editeur, n), y=n)) +
    geom_bar(stat="identity", fill="#32648E", alpha=.6, width=.5) +
    coord_flip() +
    xlab("Editeur")+
    ylab("Nombre")

edit
```

Avant 2020
==================================================

```{r}
ante20 <- data %>% 
  filter(année_fin == "Avant 2020")
```


Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbrante20 <- ante20 %>% 
        reframe(n())

valueBox(
  value = nbrante20, 
  icon = "fa-book")

```
### Nombre de pages lues

```{r}

nbrpagesante20 <- ante20 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpagesante20, 
  icon = "fa-file")

```

Row
--------------------------------------------------

### Top des autrices et des auteurs

```{r}
auteursante20 <- ante20 %>% 
  group_by(Auteur) %>% 
  count(Auteur) %>% 
  arrange(desc(n)) %>% 
  head(28)

packingante20 <- circleProgressiveLayout(auteursante20$n, sizetype="area")

auteursante20 <- cbind.data.frame(auteursante20, packingante20)

myPlotCordante20 <- circleLayoutVertices(packingante20)

plante20 <- ggplot() + 
  geom_polygon(data = myPlotCordante20, aes(x, y, group = id, fill= as.factor(id))) + 
  geom_text(data = auteursante20, aes(x, y, size=n, label = paste0(Auteur), text = paste("Auteur : ", Auteur, "<br>", "Nombre : ", n))) + 
  scale_size_continuous(range = c(2,4)) + 
  coord_equal() + 
  theme_void() +
  theme(legend.position="none",
        panel.border = element_blank(),
        panel.grid.major = element_blank()) + 
 scale_fill_viridis(discrete = T, option = "viridis")


ggplotly(plante20, tooltip = "text")
```

### Nationalités des auteurs et autrices

```{r}
natioante20 <- ante20 %>% 
  group_by (Nationalité) %>%
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hcante20 <- natioante20 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hcante20

```

### Top des maisons d'édition

```{r}
editante20 <- ante20 %>% 
  group_by(Editeur) %>% 
  count(Editeur) %>% 
  arrange(desc(n)) %>% 
  head(10) %>%
  ggplot(aes(x=reorder(Editeur, n), y=n)) +
    geom_bar(stat="identity", fill="#32648E", alpha=.6, width=.5) +
    coord_flip() +
    xlab("Editeur")+
    ylab("Nombre")

editante20
```





2020
==================================================

```{r}
an20 <- data %>% 
  filter(année_fin == "2020")
```


Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr20 <- an20 %>% 
        reframe(n())

valueBox(
  value = nbr20, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages20 <- an20 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages20, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime20 <- mean(an20$ecart_date)

moytime20 <- round(moytime20, 2)

valueBox(
  value = moytime20, 
  icon = "fa-clock")


```


Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time20 <- an20 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time20
```

### Nationalités des auteurs et autrices

```{r}
natio20 <- an20 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio20 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type20 <- an20 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type20

```



2021
=========================================

```{r}
an21 <- data %>% 
  filter(année_fin == "2021")
```

Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr21 <- an21 %>% 
        summarise(n())

valueBox(
  value = nbr21, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages21 <- an21 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages21, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime21 <- mean(an21$ecart_date)

moytime21 <- round(moytime21, 2)

valueBox(
  value = moytime21, 
  icon = "fa-clock")


```

Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time21 <- an21 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time21
```

### Nationalités des auteurs et autrices

```{r}
natio21 <- an21 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio21 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type21 <- an21 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type21

```

2022
=========================================

```{r}
an22 <- data %>% 
  filter(année_fin == "2022")
```

Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr22 <- an22 %>% 
        summarise(n())

valueBox(
  value = nbr22, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages22 <- an22 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages22, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime22 <- mean(an22$ecart_date)

moytime22 <- round(moytime22, 2)

valueBox(
  value = moytime22, 
  icon = "fa-clock")


```

Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time <- an22 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time
```

### Nationalités des auteurs et autrices

```{r}
natio22 <- an22 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio22 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type22 <- an22 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type22

```

2023
=========================================

```{r}
an23 <- data %>% 
  filter(année_fin == "2023")
```

Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr23 <- an23 %>% 
        summarise(n())

valueBox(
  value = nbr23, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages23 <- an23 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages23, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime23 <- mean(an23$ecart_date)

moytime23 <- round(moytime23, 2)

valueBox(
  value = moytime23, 
  icon = "fa-clock")


```

Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time <- an23 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time
```

### Nationalités des auteurs et autrices

```{r}
natio23 <- an23 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio23 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type23 <- an23 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type23

```

2024
=========================================

```{r}
an24 <- data %>% 
  filter(année_fin == "2024")
```

Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr24 <- an24 %>% 
        summarise(n())

valueBox(
  value = nbr24, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages24 <- an24 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages24, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime24 <- mean(an24$ecart_date)

moytime24 <- round(moytime24, 2)

valueBox(
  value = moytime20, 
  icon = "fa-clock")


```

Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time <- an24 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time
```

### Nationalités des auteurs et autrices

```{r}
natio24 <- an24 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio24 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type24 <- an24 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type24

```

2025
=========================================

```{r}
an25 <- data %>% 
  filter(année_fin == "2025")
```

Row
--------------------------------------------------
### Nombre de livres lus 

```{r}

nbr25 <- an25 %>% 
        summarise(n())

valueBox(
  value = nbr25, 
  icon = "fa-book")

```

### Nombre de pages lues

```{r}

nbrpages25 <- an25 %>%
  filter(Pages > 0) %>%
  reframe(Pages) %>%
  sum()

valueBox(
  value = nbrpages25, 
  icon = "fa-file")

```

### Temps moyen de lecture (en jours)

```{r}
moytime25 <- mean(an25$ecart_date)

moytime25 <- round(moytime25, 2)

valueBox(
  value = moytime25, 
  icon = "fa-clock")


```

Row
--------------------------------------------------

### Temps de lecture suivant le nombre de pages

```{r}
time25 <- an25 %>% 
  ggplot(aes(x=Pages, y=ecart_date, color=Genre.auteur)) + 
    geom_point(size = 3) +
    labs(x="Nombre de pages", y = "Nombre de jours de lecture", colour = "Genre")+
    scale_colour_viridis_d(labels = c("Collectif", "Femme", "Homme"))+
    theme_minimal()

time25
```

### Nationalités des auteurs et autrices

```{r}
natio25 <- an25 %>% 
  group_by (Nationalité) %>% 
  filter(Nationalité != "N/A") %>%
  count(Nationalité) %>% 
  arrange(desc(n)) %>% 
  head(10)

options(highcharter.theme = hc_theme_smpl(tooltip = list(valueDecimals = 0)))

hc <- natio25 %>%
  hchart(
    "treemap", 
    hcaes(x = Nationalité, value = n, color = n)
    ) %>%
  hc_colorAxis(stops = color_stops(colors = viridis(100)))

hc

```

### Supports de lecture

```{r}
type25 <- an25 %>%
  count(Format) %>% 
  plot_ly(labels = ~Format, values = ~n, type = "pie",
        textposition = 'inside',
        textinfo = 'label+percent',
        insidetextfont = list(color = '#FFFFFF'),
        hoverinfo = 'text',
        marker = list(colors = c("#481568", "#55C667"), line = list(color = '#FFFFFF', width = 1)), 
        showlegend = FALSE) %>% 
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

type25

```

Cartographie
=========================================

Column {.sidebar data-width=350}
------------------------------------------------

Cette carte présente le nombre d'ouvrages lus par pays en fonction de la nationalité des auteur·ices. L'objectif dans les années à venir est d'élargir les lectures aux auteur·ices africain·es.

Row {data-width=650}
------------------------------------------------

### Carte

```{r}

datam <- data %>% 
  group_by(code.ISO.3166.1.alpha.3) %>% 
  count(code.ISO.3166.1.alpha.3) %>% 
  rename("ISO3" = "code.ISO.3166.1.alpha.3")

map <- read_sf("TM_WORLD_BORDERS_SIMPL-0.3.shx")

```

```{r}
datamap <- map %>% 
  left_join(datam, by = "ISO3")

```


```{r}
bins <- c(0, 5, 10, 50, 100, 200, 400)
pal <- colorBin("RdPu", domain = datamap$n, bins = bins)
```


```{r}
m <- leaflet(datamap)%>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons( stroke = FALSE, 
               fillOpacity = 0.5, 
               smoothFactor = 0.5, 
               popup = ~paste0(NAME, "<br/>", "Nombre de livres lus : ", round(n)),
                fillColor = ~pal(n))

m
```


Données
=========================================


### Recherche dans les données {data-width=150}

```{r filter-section}

filter_select(
    id = "Auteur",
    label = "Auteur·ice",
    sharedData = st,
    group = ~Auteur
)

filter_checkbox(
    id ="Année_début",
    label = "Année",
    sharedData = st,
    group = ~Année_début

)

```

### Données 

```{r filterable_table}

datatable(st)

```

